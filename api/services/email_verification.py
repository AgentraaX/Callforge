...
from shared.constants import (
    ACCOUNT_DELETE_CODE_KEY,
    EMAIL_VERIFICATION_KEY,
    PASSWORD_CHANGE_CODE_KEY,
)
...
# ---------------------------------------------------------------------------
# Password-change verification code flow
# ---------------------------------------------------------------------------


async def store_pending_password_change(email: str, code: str) -> None:
    redis = get_redis()
    payload = json.dumps({"code": code})
    await redis.set(PASSWORD_CHANGE_CODE_KEY.format(email=email), payload, ex=_CODE_TTL_SECONDS)


async def get_pending_password_change(email: str) -> dict | None:
    """Non-destructive read - a wrong code attempt (or a pre-check via the
    verify-code endpoint) shouldn't burn the user's one attempt, so this
    doesn't delete the key. Only consume_pending_password_change (called
    once the code is confirmed correct during reset) does."""
    redis = get_redis()
    raw = await redis.get(PASSWORD_CHANGE_CODE_KEY.format(email=email))
    if raw is None:
        return None
    return json.loads(raw)


async def consume_pending_password_change(email: str) -> dict | None:
    redis = get_redis()
    raw = await redis.getdel(PASSWORD_CHANGE_CODE_KEY.format(email=email))
    if raw is None:
        return None
    return json.loads(raw)


def _send_password_change_email_sync(email: str, code: str) -> None:
    host = os.getenv("SMTP_HOST")
    port = int(os.getenv("SMTP_PORT", "587"))
    username = os.getenv("SMTP_USERNAME")
    password = os.getenv("SMTP_PASSWORD")
    ...
    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Your CallForge password change code: {code}"
    msg["From"] = from_email
    msg["To"] = email

    text_body = (
        f"Your CallForge password change code is: {code}\n\n"
        f"This code expires in 10 minutes.\n\n"
        f"If you did not request a password change, ignore this email and your password will stay the same."
    )
    html_body = (
        f"<p>Your CallForge password change code is:</p>"
        f"<p style='font-size:24px;font-weight:bold;letter-spacing:4px'>{code}</p>"
        f"<p>This code expires in 10 minutes.</p>"
        f"<p>If you did not request a password change, ignore this email and your password will stay the same.</p>"
    )
    msg.attach(MIMEText(text_body, "plain"))
    msg.attach(MIMEText(html_body, "html"))
...
async def send_password_change_email(email: str, code: str) -> None:
    await asyncio.to_thread(_send_password_change_email_sync, email, code)


async def send_password_change_code(email: str) -> str:
    """Generate, store, and email a one-time password-change code.

    Returns the generated code so callers can log it in test environments if
    needed. In production the code is delivered only via email.
    """
    code = generate_verification_code()
    await store_pending_password_change(email, code)
    await send_password_change_email(email, code)
    return code
